 ...
 * @author ryan
 */
public class MaterialImageServiceImpl extends MaterialBaseService implements IMaterialFacotryService{
	...
	public Map<String, Object> uploadMaterial(MultipartFile multipartFile) throws IllegalStateException, IOException {
		...
		if (multipartFile.getSize() > MaterialFileUtil.getFileSizeLimit(3)) { // 限制文件大小3mb
			results.put("msg", "上传文件不能超过3M");
		} else{
			String fileName = multipartFile.getOriginalFilename();
			String suffix = fileName.substring(fileName.lastIndexOf(".") + 1);
			if (!"".equals(suffix) && (suffix.toLowerCase().equals("jpg")) || (suffix.toLowerCase().equals("png"))
					|| (suffix.toLowerCase().equals("gif"))) {
				Long time = System.currentTimeMillis();
				String srcName = MD5Util.getMD5(time + fileName) + "." + suffix;
				String path = imagePath + "/" + srcName;
				...
				try{
					...
					materialMain.setMaterialSrc(getImageUrl + "/" + srcName);
					materialMain.setMaterialStatus(0);
					materialMain.setMaterialType(1); // 图片
				...
				results.put("msg", "上传文件格式不正确，文件只能为（jpg,png,gif）");
			}
		}
		return results;
	}
	...
	public int countMaterials() {
		return materialMainService.count(1);
	}
	...
	public boolean deleteMaterial(String[] params) {
		try {
			for(String id : params){
				...
				boolean flag = MaterialFileUtil.delFileByPath(imagePath + "/" + materialMain.getMaterialSysName());
				if(flag)
