...
<title>新增</title>
<!-- <link rel="stylesheet" href="/templatelibs/bootstrap/css/bootstrap.min.css">
<script src="/templatelibs/vendors/jquery-1.9.1.min.js"></script>
...
<body>
							...
							<div id="photoId"  style="padding-left: 150px">
								...
								<div id="photo-group${s.count }" style="padding-left: -200px;">
									...
									<div class="row">
										<div class="span4">
											<div class="control-group">
												<label class="control-label" for="name">头像名称${s.count }</label>
												<div class="controls">
											...
											<div class="control-group">
												<label class="control-label" for="fileInput">头像图片${s.count }</label>
												<div class="controls ">
													<div class="uploader">
														<input class="input-file uniform_on" onchange="uploadHeadPhoto(${s.count });" name="files"
															id="headPhotoId${s.count }" type="file">
													</div>
												</div>
											</div>
											<div class="control-group">
												...
												<div class="controls">
													<div class="uploader">
														<input class="input-file uniform_on" onchange="uploadQrcodePhoto(${s.count });" name="files"
															id="qrcodePhotoId${s.count }" type="file">
													</div>
												</div>
											</div>
										</div>
										<div class='span8'>
											...
											<div class='span2'>
												<h5>头像图片</h5>
												<img id="headImgId${s.count }" src='${imagesUrl }${d.detail_main_img}' width='120' height='100'>
											</div>
											<div class='span4'>
												<h5>二维码图片</h5>
												<img id="qrcodeImgId${s.count }" src='${imagesUrl}${d.detail_other_img }' width='120' height='100'>							</div> 
											</div> 
										</div>								
									</div>
	...
	<script type="text/javascript">
		function addPhoto(){
			...
			c+=				"<label class='control-label' for='name'>头像名称"+ size +"</label>";
			...
			c+=			"</div>";
			c+=			"<div class='control-group'>";
			c+=				"<label class='control-label' for='fileInput'>头像图片"+ size +"</label>";
			c+=				"<div class='controls'>";
			c+=					"<div class='uploader' id='uniform-fileInput'>";
			c+=						"<input class='input-file uniform_on' name='files' onchange='uploadHeadPhoto("+ size +")' id='headPhotoId"+ size +"' type='file'>";
			c+=					"</div>";
			c+=				"</div>";
			c+=			"</div>";
			c+=			"<div class='control-group'>";
			...
			c+=				"<div class='controls'>";
			c+=					"<div class='uploader' id='uniform-fileInput'>";
			c+=						"<input class='input-file uniform_on' name='files' onchange='uploadQrcodePhoto("+ size +")' id='qrcodePhotoId"+ size +"' type='file'>";
			c+=					"</div>";
			c+=				"</div>";
			c+=			"</div>";
			c+=			"</div>";
			c+=			"<div class='span8'>"
			c+=				"<div class='row'>"
			c+=					"<div class='span1'></div>"
			c+=					"<div class='span2'>"
			c+=						"<h5>头像图片</h5>"
			c+=						"<img id='headImgId"+ size +"' src='' width='120' height='100'>";
			c+=					"</div>"
			c+=					"<div class='span4'>"
			c+=						"<h5>二维码图片</h5>"
			c+=						"<img id='qrcodeImgId"+ size +"' src='' width='120' height='100'>";
			c+=					"</div>" 
			c+=				"</div>" 
			c+=			"</div>";
			c+=		"</div>"
			...
			c+="</div>";
			$("#photoId").append(c);
			$("#headPhotoId"+size).uniform();
			$("#qrcodePhotoId"+size).uniform();
			changeFileBtn();
		}
	</script>
	...
	<script type="text/javascript">
		...
		function savePhoto(num){
			...
			if(flag){
				...
				var headImgUrl = headSrc.substring(headSrc.lastIndexOf("images/")+7,headSrc.length);
				var qrcodeSrc = $("#qrcodeImgId"+num).attr("src");
				var qrcodeImgUrl = qrcodeSrc.substring(qrcodeSrc.lastIndexOf("images/")+7,qrcodeSrc.length);
				$.ajax({
	...
	<script src="templatelibs/vendors/jGrowl/jquery.jgrowl.js"></script> -->
